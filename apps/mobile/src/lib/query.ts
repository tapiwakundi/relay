...
import type { Channel, ChatMessage, Member, Workspace, WsServerEvent } from "@relay/shared";
...
export type MeResponse = { user: Me; workspace: Workspace | null };
...
export function applyWsEvent(ev: WsServerEvent, meId: string) {
  ...
  if (ev.type === "workspace.updated") {
    queryClient.setQueryData<MeResponse>(keys.me, (old) => (old ? { ...old, workspace: ev.workspace } : old));
    queryClient.setQueriesData<Bootstrap>({ queryKey: ["bootstrap"] }, (boot) =>
